...
public class EzResultSetHandler extends DefaultResultSetHandler {
    ...
    public List<Object> handleResultSets(Statement stmt) throws SQLException {
        ...
        //TODO，清理返回结果
        List<Object> objects = this.collapseSingleResultList(multipleResults);
    ...
    private Object getRowValue(ResultSetWrapper rsw, ResultMap resultMap, CacheKey combinedKey, String columnPrefix,
        ...
        if (rowValue != null) {
            ...
            if (rowValue != null && !this.hasTypeHandlerForResultObject(rsw, retType)) {
                ...
                foundValues = this.applyPropertyMappings(rsw, resultMap, metaObject, lazyLoader, columnPrefix) || foundValues;
                this.putAncestor(rowValue, resultMapId);
                foundValues = this.applyNestedResultMappings(rsw, resultMap, metaObject, columnPrefix, combinedKey, true)
                        || foundValues;
                this.ancestorObjects.remove(resultMapId);
                foundValues = lazyLoader.size() > 0 || foundValues;
                rowValue = foundValues || this.configuration.isReturnInstanceForEmptyRow() ? rowValue : null;
    ...
    private boolean applyPropertyMappings(ResultSetWrapper rsw, ResultMap resultMap, MetaObject metaObject,
        ...
        for (ResultMapping propertyMapping : propertyMappings) {
            ...
            if (propertyMapping.isCompositeResult()
                ...
                Object value = this.getPropertyMappingValue(rsw.getResultSet(), metaObject, propertyMapping, lazyLoader,
                        columnPrefix);
                ...
                if (value != null
                        || this.configuration.isCallSettersOnNulls() && !metaObject.getSetterType(property).isPrimitive()) {
    ...
    private List<UnMappedColumnAutoMapping> createAutomaticMappings(ResultSetWrapper rsw, ResultMap resultMap,
        ...
        if (autoMapping == null) {
            ...
            for (String columnName : unmappedColumnNames) {
                ...
                //TODO 原有的查找类字段名称逻辑
                //TODO final String property = metaObject.findProperty(propertyName, this.configuration.isMapUnderscoreToCamelCase());
                //TODO 改为调用自定义的查找逻辑
                final String property = this.retColumnToField(propertyName, metaObject);
                ...
                if (property != null && metaObject.hasSetter(property)) {
                    ...
                    if (typeHandler != null) {
                        ...
                        this.configuration.getAutoMappingUnknownColumnBehavior().doAction(this.mappedStatement, columnName, property,
                                propertyType);
                    }
                } else {
                    this.configuration.getAutoMappingUnknownColumnBehavior().doAction(this.mappedStatement, columnName,
    ...
    private void addPendingChildRelation(ResultSet rs, MetaObject metaResultObject, ResultMapping parentMapping)
        ...
        List<PendingRelation> relations = MapUtil.computeIfAbsent(this.pendingRelations, cacheKey, k -> new ArrayList<>());
        // issue #255
    ...
    private Object createResultObject(ResultSetWrapper rsw, ResultMap resultMap, ResultLoaderMap lazyLoader,
        ...
        //TODO 获取结果类型
        Class<?> retType = ResultMapInitLogic.getRetType(resultMap);
        if (resultObject != null && !this.hasTypeHandlerForResultObject(rsw, retType)) {
            final List<ResultMapping> propertyMappings = resultMap.getPropertyResultMappings();
            for (ResultMapping propertyMapping : propertyMappings) {
                // issue gcode #109 && issue #149
                if (propertyMapping.getNestedQueryId() != null && propertyMapping.isLazy()) {
                    resultObject = this.configuration.getProxyFactory().createProxy(resultObject, lazyLoader, this.configuration,
                            this.objectFactory, constructorArgTypes, constructorArgs);
                    break;
                }
            }
        }
    ...
    private Object createResultObject(ResultSetWrapper rsw, ResultMap resultMap, List<Class<?>> constructorArgTypes,
        ...
        if (!constructorMappings.isEmpty()) {
            return this.createParameterizedResultObject(rsw, resultType, constructorMappings, constructorArgTypes, constructorArgs,
                    columnPrefix);
        } else if (resultType.isInterface() || metaType.hasDefaultConstructor()) {
            return this.objectFactory.create(resultType);
        } else if (this.shouldApplyAutomaticMappings(resultMap, false)) {
            return this.createByConstructorSignature(rsw, resultMap, columnPrefix, resultType, constructorArgTypes,
                    constructorArgs);
        }
        ...
        for (ResultMapping constructorMapping : constructorMappings) {
            ...
            try {
                if (constructorMapping.getNestedQueryId() != null) {
                    ...
                    String constructorColumnPrefix = this.getColumnPrefix(columnPrefix, constructorMapping);
                    final ResultMap resultMap = this.resolveDiscriminatedResultMap(rsw.getResultSet(),
                            this.configuration.getResultMap(constructorMapping.getNestedResultMapId()), constructorColumnPrefix);
                    value = this.getRowValue(rsw, resultMap, constructorColumnPrefix);
                } else {
    ...
    private Object createByConstructorSignature(ResultSetWrapper rsw, ResultMap resultMap, String columnPrefix,
        ...
        return this.applyConstructorAutomapping(rsw, resultMap, columnPrefix, resultType, constructorArgTypes, constructorArgs,
                this.findConstructorForAutomapping(resultType, rsw).orElseThrow(() -> new ExecutorException(
    ...
    private Optional<Constructor<?>> findConstructorForAutomapping(final Class<?> resultType, ResultSetWrapper rsw) {
        ...
        if (this.configuration.isArgNameBasedConstructorAutoMapping()) {
            ...
            return Arrays.stream(constructors).filter(x -> this.findUsableConstructorByArgTypes(x, rsw.getJdbcTypes())).findAny();
        }
    }
    ...
    private Object applyConstructorAutomapping(ResultSetWrapper rsw, ResultMap resultMap, String columnPrefix,
        ...
        boolean foundValues = false;
        if (this.configuration.isArgNameBasedConstructorAutoMapping()) {
            foundValues = this.applyArgNameBasedConstructorAutoMapping(rsw, resultMap, columnPrefix, constructorArgTypes,
                    constructorArgs, constructor, foundValues);
        } else {
            foundValues = this.applyColumnOrderBasedConstructorAutomapping(rsw, constructorArgTypes, constructorArgs, constructor,
                    foundValues);
        }
    ...
    private boolean applyArgNameBasedConstructorAutoMapping(ResultSetWrapper rsw, ResultMap resultMap,
            ...
            for (String columnName : rsw.getColumnNames()) {
                if (this.columnMatchesParam(columnName, paramName, columnPrefix)) {
                    ...
                    if (!this.autoMappingsCache.containsKey(mapKey)) {
                        MapUtil.computeIfAbsent(this.constructorAutoMappingColumns, mapKey, k -> new ArrayList<>()).add(columnName);
                    }
        ...
        if (foundValues && constructorArgs.size() < params.length) {
            throw new ExecutorException(MessageFormat.format(
                    ...
                    missingArgs, constructor, rsw.getColumnNames(), this.configuration.isMapUnderscoreToCamelCase()));
        }
        return foundValues;
    }
    ...
    private Object getNestedQueryConstructorValue(ResultSet rs, ResultMapping constructorMapping, String columnPrefix)
        ...
        final Object nestedQueryParameterObject = this.prepareParameterForNestedQuery(rs, constructorMapping,
                nestedQueryParameterType, columnPrefix);
        Object value = null;
        if (nestedQueryParameterObject != null) {
            final BoundSql nestedBoundSql = nestedQuery.getBoundSql(nestedQueryParameterObject);
            final CacheKey key = this.executor.createCacheKey(nestedQuery, nestedQueryParameterObject, RowBounds.DEFAULT,
                    nestedBoundSql);
            final Class<?> targetType = constructorMapping.getJavaType();
            final ResultLoader resultLoader = new ResultLoader(this.configuration, this.executor, nestedQuery,
    ...
    private Object getNestedQueryMappingValue(ResultSet rs, MetaObject metaResultObject, ResultMapping propertyMapping,
        ...
        final MappedStatement nestedQuery = this.configuration.getMappedStatement(nestedQueryId);
        final Class<?> nestedQueryParameterType = nestedQuery.getParameterMap().getType();
        final Object nestedQueryParameterObject = this.prepareParameterForNestedQuery(rs, propertyMapping,
                nestedQueryParameterType, columnPrefix);
        Object value = null;
        if (nestedQueryParameterObject != null) {
            final BoundSql nestedBoundSql = nestedQuery.getBoundSql(nestedQueryParameterObject);
