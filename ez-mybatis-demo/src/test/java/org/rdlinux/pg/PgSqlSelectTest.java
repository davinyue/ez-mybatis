...
@Slf4j
public class PgSqlSelectTest extends PgSqlBaseTest {
    ...
    public void ezQueryConcurrentSelectRetTypeTest() throws InterruptedException, java.util.concurrent.ExecutionException {
        this.ensureData(PgSqlBaseTest.sqlSessionFactory.openSession());
    ...
    public void ezQueryConditions() {
        ...
        EzQuery<User> betweenQuery = EzQuery.builder(User.class).from(userTable)
                .select().addAll().done()
                .where().addCondition(userTable.field(User.Fields.age).between(10, 30)).done()
                .page(1, 1).build();
        ...
        EzQuery<User> notBetweenQuery = EzQuery.builder(User.class).from(userTable)
                .select().addAll().done()
                .where().addCondition(userTable.field(User.Fields.age).notBetween(100, 200)).done()
                .page(1, 1).build();
    ...
    public void ezQueryUnion() {
        ...
        EzQuery<User> unionQuery = EzQuery.builder(User.class).from(userTable)
                .select().addAll().done()
                .where().addCondition(userTable.field(User.Fields.name).eq("NonExistent")).done() // Main query empty
                .union(q1)
                .unionAll(q2)
                .page(1, 10)
                .build();
    ...
    public void ezQueryFunctionsAndFormulas() {
        ...
        // Formula: age + 1
        Formula agePlusOne = Formula.builder(EntityField.of(table, User.Fields.age)).add(1).done().build();
    ...
    public void ezQueryCaseWhen() {
        ...
        CaseWhen ageGroup = CaseWhen.builder(table)
                .when().addFieldCondition(table, User.Fields.age, Operator.lt, 18).then("Young")
                .when().addFieldCondition(table, User.Fields.age, Operator.ge, 18).then("Adult")
                .els("Unknown");
