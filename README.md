...
- 需要 **类型安全查询构建器** 避免字段写错？
- 原生 JOIN 支持，无需手写复杂 SQL，Ez-MyBatis 提供了完整的连表查询、连表删除等 DSL，支持 INNER JOIN / LEFT JOIN / RIGHT
  JOIN 等常用连接方式，并且提供极简的 **Lambda DSL**。
...
- 🎯 **类型安全 DSL**：全面支持 Lambda 风格 DSL 以构建 CRUD
  操作，并且支持复杂表达式（Formula）、函数（Function）、条件分支（CaseWhen）及窗口函数（WindowFunction）。
...
Ez-MyBatis 会自动根据数据源的 Driver Class 识别数据库类型，通常情况下**无需手动配置**。对于无法自动识别的数据库（如高斯），可配置为兼容的类型（示例：配置为
POSTGRE_SQL）：
...
batchInsert(complexUsers);
ezMapper.

insertByTable(EntityTable.of(ComplexUser.class),complexUser);
...
insertByQuery(archiveTable, query);
...
支持对象级全体更新、部分字段更新，以及依托复杂条件或 `CASE WHEN` / `Formula` 构建的灵活表达式更新。另外支持如 Oracle 的
`MERGE INTO` 操作。
...
ezUpdate(caseWhenUpdate);

// 4. 批量执行多个 EzUpdate
ezMapper.

ezBatchUpdate(Arrays.asList(ezUpdate, funcUpdate, caseWhenUpdate));
...
deleteById(ComplexUser .class, "user_1");
ezMapper.

batchDeleteById(ComplexUser .class, Arrays.asList("user_1", "user_2"));
...
addQueryRetListener(config, new EzMybatisQueryRetListener() {
    @Override
